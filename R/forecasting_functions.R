...
#' @export
forecast_okun_ssm_old <- function(params_df,                  
                              date_col = "quarter",
...
#' Forecast Okun SSM (Adjusted for State-Deterministic Cycle Layout)
...
forecast_philips_ssm <- function(params_df,
  ...
  for (i in seq_along(dates)) {
    ...
    # --- ADJUSTED METRIC PARAMETER MAPPING ---
    pi_bar  <- current_params$natural_rate 
    beta_y  <- current_params$beta_y
    psi_lop <- current_params$psi_lop
    phi     <- current_params$phi
    
    # Starting historical anchor for the AR recursive projection loop
    y_t_t <- master_df %>% 
      filter(quarter == forecast_origin) %>% 
      pull(log_inflation_diff)
    ...
    X_future <- gdp_forecasts %>%
      ...
      left_join(lop_forecast %>% select(quarter, lop_gap), by = "quarter") %>%
      filter(quarter > forecast_origin) %>%
      # Scale gaps to perfectly mirror the estimation setup parameters
      mutate(
        gdp_gap = as.numeric(gdp_gap) * 100,
        lop_gap = as.numeric(lop_gap) * 100
      )
    ...
    if (h_available > 0 && length(y_t_t) > 0) {
      ...
      for (h in 1:h_available) {
        ...
        # Pull structural drivers for horizon step h
        lop_gap_h <- exog_now$lop_gap 
        gdp_gap_h <- exog_now$gdp_gap
        ...
        # Compute the autoregressive deviation tracking factor
        inf_deviation <- current_inflation - pi_bar 
        
        # Combine steady-state drift, momentum persistence, and cyclical gaps
        pred_inflation <- pi_bar + (phi * inf_deviation) + cyclical_inflation
        
        # Store prediction step value in evaluation matrix
        eval_mat[fdate_str, i] <- pred_inflation
        
        # Feed prediction back as the baseline anchor for step h + 1
        current_inflation <- pred_inflation 
      }
    }
  }
